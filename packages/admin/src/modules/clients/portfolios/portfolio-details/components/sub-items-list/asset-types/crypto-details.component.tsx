...
export const CryptoDetails: React.FC<IAssetDrawerProps> = ({
	asset,
},) => {
	const parsedPayload = JSON.parse(asset.payload,)
	const isCryptoETF = parsedPayload.productType === CryptoType.ETF
	...
	return (
		<div className={styles.drawerContent}>
			...
			<div className={styles.drawerItemBorder}>
				{renderField('Product ype', parsedPayload.productType,)}
				{isCryptoETF && (
					<>
				...
				{!isCryptoETF && (
					<>
						{renderField('Crypto currency type', parsedPayload.cryptoCurrencyType,)}
						{renderField('Amount in crypto', formatWithAllDecimals(parsedPayload.cryptoAmount,),)}
						{renderField('Exchange / Wallet', parsedPayload.exchangeWallet,)}
						{renderField('Date of purchase', parsedPayload.purchaseDate && new Date(parsedPayload.purchaseDate,).toLocaleDateString('en-GB', {
							day:   '2-digit',
							month: '2-digit',
							year:  'numeric',
						},)
							.replace(/\//g, '.',),)}
						{renderField('Purchase price', formatWithAllDecimals(parsedPayload.purchasePrice,), hasComment,)}
					</>
				)}
