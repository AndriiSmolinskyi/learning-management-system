...
export const ItemDetails: React.FC<Props> = ({
	...
	const isDatePast = row.productType === CryptoType.ETF && row.valueDate ?
		isDateFromPast(row.valueDate,) :
		isDateFromPast(row.purchaseDate,)
						...
						<div className={assetStyles.menuActions}>
							...
							{isAllowed && (
								<>
									...
									/>}
									{!isVersion && isDatePast && !row.isTransferred && <Button
										className={cx(Classes.POPOVER_DISMISS, assetStyles.actionBtn,)}
										additionalProps={{
											btnType:  ButtonType.TEXT,
											size:     Size.MEDIUM,
									...
									/>}
									{!isVersion && !row.isTransferred && <Button<ButtonType.TEXT>
										className={cx(Classes.POPOVER_DISMISS, assetStyles.actionBtn,)}
										onClick={() => {
