...
export const TableMockupItem: React.FC<Props> = ({
	...
	return (
		<tr className={styles.tableMockupRow()}>
			...
			<td className={styles.tableCell}>
				<div className={styles.cellContent}>{parsedPayload.investmentAssetName ?? 'N/A'}</div>
			</td>
			<td className={styles.tableCell}>
				<div className={styles.cellContent}>{parsedPayload.serviceProvider ?? 'N/A'}</div>
			</td>
			<td className={styles.tableCell}>{parsedPayload.currency ?? 'N/A'}</td>
			<td className={cx(styles.tableCell, styles.tableNumberField,)}>{localeString(Number(parsedPayload.currencyValue ?? 0,), '', 0, false,)}</td>
			<td className={cx(styles.tableCell, styles.textNowrap, styles.tableNumberField,)}>
				{parsedPayload.investmentDate ?
					format(parsedPayload.investmentDate, 'dd.MM.yyyy',) :
					'N/A'}
			</td>
			<td className={cx(styles.tableCell, styles.tableNumberField,)}>{localeString(parsedPayload.usdValue ?? 0, 'USD', 0, false,)}</td>
			<td className={cx(styles.tableCell, styles.tableNumberField,)}>{localeString(parsedPayload.marketValueUsd ?? 0, 'USD', 0, false,)}</td>
